...
public class FeedbackController {
    ...
    public String newFeedback(Feedback feedback, HttpSession session, HttpServletResponse response) {
        User user = (User) session.getAttribute("user");
        feedback.setUserId(user.getId());
        feedback.setSubmitTime(new Date());
        feedbackService.submit(feedback);
        CookieUtil.addMessage(response, "user", new Message(Message.TYPE_SUCCESS, "反馈已经发送"), "/");
